...
// --- API Utility ---
// Robust API key retrieval to prevent build-time failures
const getApiKey = () => {
  try {
    // This safely checks for Vite environment variables during build/runtime
...
async function callGemini(prompt, systemInstruction = "") {
  if (!apiKey) {
    return "The Spark Oracle is currently in 'Safe Mode' because the API Key is not yet set in Vercel Environment Variables. Please add VITE_GEMINI_API_KEY to your Vercel settings.";
  }
      ...
      <section className="py-48 px-6 text-center bg-white/5 border-t border-white/5">
        <FadeIn>
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <button onClick={() => setIsRegisterOpen(true)} className="px-12 py-6 bg-white text-black rounded-3xl font-black text-2xl uppercase tracking-tighter hover:bg-indigo-50 shadow-2xl transition-all">Join the Network</button>
